...
Většinu dne bude hlavní řídicí jednotka v režimu Stop2 s RTC. Tento režim se vyznačuje velmi nízkou spotřebou a na rozdíl od režimu StandBy s RTC dokáže mimo jiné udržet logické úrovně a nastavení pinů. Řadič je taktován externím krystalem LSE, umístěným na Wio-E5 mini, na 32 kHz. Jakmile ale RTC hodiny signalizují, že je čas na práci, přepne se řadič do režimu LP Run (Low-Power Run), taktovaného interním krystalem MSI na 1 MHz. Pro složitý výpočet astronomických hodin řadič zvolí strategii Race-to-Sleep. Ta spočívá v přepnutí do méně úsporného, ale rychlejšího režimu Run (HSI, 48 MHz) po velmi krátkou dobu jedné milisekundy. V průběhu přenosu dat (Radio TX/RX) se CPU přepne do režimu LP Sleep (MSI, 1 MHz); rádio poběží automaticky na 32 MHz a po skončení přenosu bude nastaveno do režimu LP Sleep. Kvůli nízké taktovací frekvenci je potřeba v header souboru lorawan_conf.h zvýšit RADIO_WAKEUP_TIME z 2 na 5 ms. Při režimech LP Run a LP Sleep je potřeba snížit napětí interního regulátoru — HAL_PWREx_ControlVoltageScaling(PWR_REGULATOR_VOLTAGE_SCALE2). Tento řadič bude využívat úsporného SMPS napájecího režimu — HAL_PWREx_ConfigSupply(PWR_SMPS_SUPPLY).
...
Kvůli nízkopříkonové povaze systému bude nutné u Wio-E5 mini odpájet zelenou User LED diodu, TX LED diodu, RX LED diodu, shottkyho diodu a lineární LDO regulátor; u ostatních řadičů bude nutné odpájet červenou Power LED diodu a pájecí můstky SB2, SB3, SB9, SB14 a SB15 (LED diody, lineární LDO regulátor a interní programátor). Zvláštní pozornost je třeba věnovat plovoucím pinům — nepoužívané piny musí být vždy v analogovém režimu bez pull rezistoru. Nakonec je potřeba u Wio-E5 mini nastavit TCXO oscilátor (pin PB0) a při nepoužívání rádia externí RF switch (piny PA4 a PA5) na logickou nulu a u ostatních řadičů v power registrech (PWR) zapnout ultra low power režim (ULP bit) a vypnout fast wake up (FWU bit).
