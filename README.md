...
*Poznámka: Ostatní části systému jsou odpojovány přes tranzistorové spínače. (Součet proudu opraven: 2 + 1,0 + 1,9 = 4,9 µA typ; 5 + 2,8 + 1,9 = 9,7 µA max — energetické součty 118 / 233 µAh byly správně již v původní verzi.)*
...
| **Celkem** | **0,92 mA** | **1,22 mA** | **11,5 µAh** | **15,3 µAh** |

*Poznámka: Součet spotřeby opraven — 2,75 + 8,75 = 11,5 µAh (typ), 2,75 + 12,5 = 15,3 µAh (max, původně chybně 14,6 µAh). Čas v závorce byl opraven z původního "4 s a 3 s" na 45 s — právě tento čas totiž odpovídá uvedeným hodnotám spotřeby u obou položek (2,75 µAh / 220 µA = 8,75 µAh / 700 µA = 12,5 µAh / 1 mA = 45 s).*

### Pohyb dvířek (cca 40 s typ, cca 60 s max)

| Komponenta | Proud (typ) | Proud (max) | Spotřeba (typ) | Spotřeba (max) |
|:---|:---:|:---:|:---:|:---:|
| Motor | 150 mA | 200 mA | 1,67 mAh | 3,33 mAh |
| DRV8838 | 340 µA | 600 µA | 3,78 µAh | 10,0 µAh |
| M (LPRun @ 1 MHz) | 220 µA | 220 µA | 2,44 µAh | 3,67 µAh |
| INA219 | 0,7 mA | 1 mA | 7,78 µAh | 16,7 µAh |
| **Celkem** | **151 mA** | **202 mA** | **1,68 mAh** | **3,36 mAh** |

*Poznámka: Možnost zaseknutí dvířek je brána v potaz. Čas v závorce byl opraven — typický pohyb odpovídá cca 40 s (ne 28 s), horší případ (zaseknutí, zastavení na 250 ms, otočení motoru, návrat) cca 60 s (ne 42 s). Hodnoty spotřeby u DRV8838, M a INA219 ve sloupci max byly přepočítány na stejných 60 s jako motor (dříve odpovídaly jen ~40 s, tedy jako by tyto obvody po dobu prodlouženého zásahu neběžely déle než při běžném otevření): DRV8838 600 µA × 60 s = 10,0 µAh, M 220 µA × 60 s = 3,67 µAh, INA219 1 mA × 60 s = 16,7 µAh. Nový součet spotřeby (max) = 3,33 + 0,0100 + 0,00367 + 0,0167 mAh ≈ 3,36 mAh. Proud (max) zůstává 202 mA beze změny, protože jde o okamžitou hodnotu, nikoli o energii.*
...
*Poznámka: Součty souhlasí. Čas v závorce opraven z 4 s na 3 s — uvedeným hodnotám spotřeby odpovídá přesně 3 s (17,5 µAh / 21 mA = 0,0367 µAh / 44 µA = 0,0575 µAh / 69 µA = 3 s).*
...
*Poznámka: Bezpečný odhad délky okna je 200 ms. Součty souhlasí. Čas v závorce opraven z 11 s na 10 s — uvedeným hodnotám spotřeby odpovídá přesně 10 s (13,3 µAh / 4,8 mA = 0,122 µAh / 44 µA = 0,192 µAh / 69 µA = 10 s).*

### Procentuální rozložení a celková denní spotřeba

| Blok | Spotřeba (typ) | Podíl | Spotřeba (max) | Podíl |
|:---|:---:|:---:|:---:|:---:|
| Pohyb dvířek | 1,68 mAh | 61,0 % | 3,36 mAh | 69,3 % |
| Kontrola vajec | 914 µAh | 33,2 % | 1,21 mAh | 24,9 % |
| Klidový režim | 118 µAh | 4,3 % | 233 µAh | 4,8 % |
| LoRa TX | 17,5 µAh | 0,6 % | 17,6 µAh | 0,4 % |
| LoRa RX | 13,4 µAh | 0,5 % | 13,5 µAh | 0,3 % |
| Kontrola panelu / baterie | 11,5 µAh | 0,4 % | 15,3 µAh | 0,3 % |
| **Celkem** | **2,75 mAh** | 100 % | **4,85 mAh** | 100 % |

*Poznámka: Podíly u řádku Klidový režim byly opraveny (typ 4,3 % místo 3,9 %, max 4,8 % místo 4,3 %) — hodnota 118 µAh, resp. 233 µAh totiž z celkového denního součtu tvoří právě tento podíl. Řádek Kontrola panelu / baterie přebírá opravenou hodnotu spotřeby (15,3 µAh) z předchozí tabulky. Řádek Pohyb dvířek (max) byl navýšen na 3,36 mAh podle přepočtu DRV8838, M a INA219 na 60 s (viz poznámka u dané tabulky), což mírně zvýšilo celkovou denní spotřebu (max) na 4,85 mAh (dříve 4,84 mAh) a odpovídajícím způsobem posunulo podíly: Pohyb dvířek 69,3 % (dříve 69,2 %), Kontrola vajec 24,9 % (dříve 25,0 %). Ostatní podíly zůstávají beze změny. Součet podílů po opravě dává přesně 100 % jak u typ, tak u max sloupce.*
...
*Poznámka: Do posledních dvou tabulek byly započteny ztráty neideálního pracovního bodu fotovoltaického panelu a jednoduchého regulátoru, přibližná nabíjecí účinnost akumulátoru, samovybíjení a běžné klimatické podmínky v ČR. Kontrolní přepočet (Wh/6 V → mAh, porovnáno s uvedenou bilancí) odpovídá implikované účinnosti přibližně 87–90 % v létě a 80–88 % v zimě, což je v souladu s popsanými ztrátami a se zimním poklesem kapacity — čísla jsou tedy vzájemně konzistentní. Přesný podíl jednotlivých ztrát (účinnost regulátoru, samovybíjení, teplotní derating) text číselně nespecifikuje, proto tuto tabulku nelze ověřit do poslední platné číslice, pouze potvrdit, že je vnitřně bezrozporná.*
...
Senzor INA219 bude zároveň sloužit jako proudový snímač s 12bitovým rozlišením a průměrováním 4 vzorků. Při pohybu dvířek bude M monitorovat proud odebíraný z akumulátoru; náhlé zvýšení proudu nad 225 mA u tří po sobě jdoucích měření bude signalizovat zablokování dvířek nebo překážku v cestě (typicky slepici). V takovém případě se M na 250 ms zastaví, pokusí se obrátit směr otáčení motoru a vrátit dvířka do původní polohy, poté se uspí a po 10 minutách pokus zopakuje. Nepomůže-li ani zpětný chod, systém se vypne. Krátkodobé proudové špičky při rozběhu motoru, trvající asi 150 ms, je nutné ignorovat.
...
https://www.gme.cz/v/1512382/elektrokabel-cya-1x05-cerveny-h05v-k-izolovany-vodic-lanko (5 m)
https://www.gme.cz/v/1512360/elektrokabel-cya-1x05-cerny-h05v-k-izolovany-vodic-lanko (5 m)
